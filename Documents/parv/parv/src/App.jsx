  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" style={{ flexShrink: 0 }}>
      ...
      <path
        ...
        strokeWidth="0.5"
      />
      {/* Inner emblem - team-specific */}
      {teamKey === "GG" && (
        <g filter={`url(#glow-${teamKey})`}>
          {/* Gladiator helmet */}
          <path d="M22 26 Q22 18 32 18 Q42 18 42 26 L42 38 Q42 44 32 46 Q22 44 22 38 Z" fill="#1a1408" stroke="#fff" strokeWidth="0.6"/>
          <rect x="24" y="28" width="16" height="2" fill="#1a1408"/>
          <rect x="24" y="32" width="16" height="2" fill="#1a1408"/>
          <path d="M28 18 Q32 10 36 18" fill="#1a1408" stroke="#fff" strokeWidth="0.6"/>
          {/* Crossed swords */}
          <line x1="14" y1="32" x2="22" y2="40" stroke="#fff" strokeWidth="1.2" strokeLinecap="round"/>
          <line x1="50" y1="32" x2="42" y2="40" stroke="#fff" strokeWidth="1.2" strokeLinecap="round"/>
        </g>
      )}
      {teamKey === "MS" && (
        <g filter={`url(#glow-${teamKey})`}>
          {/* Cricket ball with motion lines */}
          <circle cx="38" cy="24" r="6" fill="#fff" stroke="#0a1a2e" strokeWidth="0.6"/>
          <path d="M34 22 Q38 24 42 22" fill="none" stroke="#0a1a2e" strokeWidth="0.5"/>
          <path d="M34 26 Q38 24 42 26" fill="none" stroke="#0a1a2e" strokeWidth="0.5"/>
          <line x1="14" y1="20" x2="28" y2="22" stroke="#fff" strokeWidth="1" strokeLinecap="round" opacity="0.7"/>
          <line x1="14" y1="26" x2="28" y2="26" stroke="#fff" strokeWidth="1.5" strokeLinecap="round"/>
          <line x1="14" y1="32" x2="28" y2="30" stroke="#fff" strokeWidth="1" strokeLinecap="round" opacity="0.7"/>
          {/* Bat */}
          <rect x="28" y="36" width="4" height="14" rx="1" fill="#fff" transform="rotate(-30, 30, 43)"/>
          <rect x="27" y="48" width="6" height="3" rx="0.5" fill="#fff" transform="rotate(-30, 30, 49.5)"/>
        </g>
      )}
      {teamKey === "MC" && (
        <g filter={`url(#glow-${teamKey})`}>
...
export default function TournamentApp() {
  ...
  const [tab, setTab] = useState("standings"); // standings | matches | teams
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await window.storage.set(STORAGE_KEY, JSON.stringify(matches), true);
      } catch (e) {
  ...
  return (
    <div className="app-root">
      ...
      {/* LIVE MATCH BANNER */}
      {liveMatch && <LiveBanner match={liveMatch} onTap={() => setEditing(liveMatch)} />}

      {/* TABS */}
      <nav className="tabs">
        <button className={tab === "standings" ? "tab active" : "tab"} onClick={() => setTab("standings")}>
          STANDINGS
        </button>
      ...
      <main className="content">
        ...
        {tab === "matches" && (
          <Matches matches={matches} onEdit={(m) => setEditing(m)} onUpdateMatch={updateMatch} />
        )}
        {tab === "teams" && <Squads />}
      </main>
    ...
    <section className="section">
      ...
      {!allZero && (
        <div className="leader-card" style={{
          ...
          <div className="leader-team">
            ...
            <div>
              <div className="leader-name" style={{ color: TEAMS[champion.key].primary }}>
                {TEAMS[champion.key].name.toUpperCase()}
              </div>
              <div className="leader-pts">{champion.PTS} PTS · NRR {champion.NRR.toFixed(2)}</div>
            </div>
      ...
      <div className="table-wrap">
        <div className="ptable">
          ...
          {standings.map((row, i) => (
            <div key={row.key} className="ptable-row" style={{
              background: i === 0 && !allZero
              ...
              <div className="col-team">
                ...
                <span style={{ color: TEAMS[row.key].primary }}>{TEAMS[row.key].short}</span>
              </div>
              ...
              <div className="col-num bold pts" style={{ color: TEAMS[row.key].primary }}>{row.PTS}</div>
              <div className="col-num nrr">{row.NRR >= 0 ? "+" : ""}{row.NRR.toFixed(2)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rules">
        <div className="rule"><span className="rule-k">WIN</span><span className="rule-v">+2 PTS</span></div>
        <div className="rule"><span className="rule-k">TIE / NR</span><span className="rule-v">+1 PT</span></div>
        <div className="rule"><span className="rule-k">LOSS</span><span className="rule-v">0 PTS</span></div>
      </div>
    </section>
  );
}
...
// ============= LIVE BANNER =============
function LiveBanner({ match, onTap }) {
  const A = TEAMS[match.a];
  const B = TEAMS[match.b];
  return (
    <div className="live-banner" onClick={onTap}>
...
// ============= MATCHES =============
function Matches({ matches, onEdit, onUpdateMatch }) {
  return (
    <section className="section">
      <SectionTitle text="MATCH SCHEDULE" accent="#F5B800" />
      <div className="match-list">
        {matches.map((m, i) => (
          <MatchCard key={m.id} match={m} num={i + 1} onEdit={onEdit} onUpdateMatch={onUpdateMatch} />
        ))}
      </div>
    </section>
  );
...
function MatchCard({ match, num, onEdit, onUpdateMatch }) {
  const A = TEAMS[match.a];
  const B = TEAMS[match.b];
    ...
    >
      ...
      <div className="match-body">
        <div className="match-side">
          ...
          <div className="match-team-name" style={{ color: A.primary }}>{A.short}</div>
          {completed && <div className="match-score">{match.aRuns}/{match.aWickets} <span className="ov">({match.aOvers})</span></div>}
        </div>

        <div className="match-vs-col">
          <div className="match-vs">VS</div>
          {completed ? (
            <div className="match-result-pill" style={{
              borderColor: match.result === "a" ? A.primary : match.result === "b" ? B.primary : "#888",
              color: match.result === "a" ? A.primary : match.result === "b" ? B.primary : "#bbb",
            }}>{resultText}</div>
          ) : live ? (
            <div className="status-live"><span className="live-dot-sm" /> LIVE</div>
          ) : (
        ...
        <div className="match-side right">
          ...
          <div className="match-team-name" style={{ color: B.primary }}>{B.short}</div>
          {completed && <div className="match-score">{match.bRuns}/{match.bWickets} <span className="ov">({match.bOvers})</span></div>}
        </div>
      </div>
      ...
      <div className="match-actions">
        ...
        <button className="mini-btn primary" onClick={(e) => { e.stopPropagation(); onEdit(match); }}>
...
function ResultModal({ match, onClose, onSave }) {
  ...
  const reopenAsUpcoming = () => {
    onSave({
      ...
      status: "upcoming",
      result: null,
      aRuns: "", aWickets: "", aOvers: "",
      bRuns: "", bWickets: "", bOvers: "",
    });
  };
    ...
    <div className="modal-back" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <ScoreInput team={A} runs={form.aRuns} wkts={form.aWickets} overs={form.aOvers}
            onRuns={(v) => set("aRuns", v)} onWkts={(v) => set("aWickets", v)} onOvers={(v) => set("aOvers", v)} />
          <div className="vs-line">VS</div>
          <ScoreInput team={B} runs={form.bRuns} wkts={form.bWickets} overs={form.bOvers}
            onRuns={(v) => set("bRuns", v)} onWkts={(v) => set("bWickets", v)} onOvers={(v) => set("bOvers", v)} />
              ...
              >
                AUTO {computedResult && (
                  <span className="ropt-hint">
                    ({computedResult === "a" ? A.short + " WON" : computedResult === "b" ? B.short + " WON" : "TIE"})
                  </span>
                )}
              </button>
              <button
                className={form.result === "a" ? "ropt active" : "ropt"}
                style={form.result === "a" ? { borderColor: A.primary, color: A.primary } : {}}
                onClick={() => set("result", "a")}
              >{A.short} WON</button>
              <button
                className={form.result === "b" ? "ropt active" : "ropt"}
                style={form.result === "b" ? { borderColor: B.primary, color: B.primary } : {}}
        ...
        <div className="modal-foot">
          {match.status === "completed" && (
            <button className="btn ghost" onClick={reopenAsUpcoming}>Clear Result</button>
          )}
  ...
  return (
    <div className="score-row" style={{ borderColor: team.border }}>
      <div className="score-team">
        ...
        <div className="score-team-name" style={{ color: team.primary }}>{team.short}</div>
      </div>
      <div className="score-fields">
        <div className="sf">
          <label>RUNS</label>
          <input type="number" inputMode="numeric" value={runs} onChange={(e) => onRuns(e.target.value)} placeholder="0" />
        </div>
        <div className="sf small">
          <label>WKTS</label>
          <input type="number" inputMode="numeric" min="0" max="10" value={wkts} onChange={(e) => onWkts(e.target.value)} placeholder="0" />
        </div>
        <div className="sf">
          <label>OVERS</label>
          <input type="number" inputMode="decimal" step="0.1" value={overs} onChange={(e) => onOvers(e.target.value)} placeholder="0.0" />
        </div>
      </div>
    </div>
  );
    ...
    <section className="section">
      ...
      <div className="squads">
        {Object.entries(TEAMS).map(([k, t]) => (
          <div key={k} className="squad-card" style={{
            background: `linear-gradient(180deg, ${t.soft}, transparent 80%)`,
            borderColor: t.border,
          }}>
            <div className="squad-head">
              <TeamBadge teamKey={k} size={56} />
              <div className="squad-name" style={{ color: t.primary }}>{t.short}</div>
            </div>
            <ul className="squad-list">
              <li className="squad-row">
                <span className="badge" style={{ background: t.primary, color: "#0a0a0f" }}>C</span>
                <span className="player">{t.captain}</span>
              </li>
              <li className="squad-row">
                <span className="badge outline" style={{ borderColor: t.primary, color: t.primary }}>VC</span>
  ...
  return (
    <div className="modal-back" onClick={onCancel}>
      <div className="modal small" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body"><p style={{ color: "#bbb", lineHeight: 1.6 }}>{body}</p></div>
        <div className="modal-foot">
          <button className="btn ghost" onClick={onCancel}>Cancel</button>
          <button className="btn danger" onClick={onConfirm}>Reset Everything</button>
        </div>
      </div>
    </div>
  );
